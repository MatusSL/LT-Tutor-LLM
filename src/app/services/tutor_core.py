...
from app.schemas.models import TutorResult, UserInputAnalysis, TutorResponse, Language
...
class TutorCore:
    ...
    def process_user_input(self, tutor_response: TutorResponse) -> TutorResult:
        if tutor_response.input_language != Language.SPANISH:
            return TutorResult(is_correct=True, learned_words=set())
        ...
        if is_correct or len(correction.error_candidates) == 1:
            ...
            learnable_words = self.vocabulary.verify_and_update_vocabulary(
                user_input_analysis
            )

            return TutorResult(is_correct=True, learned_words=learnable_words)

        return TutorResult(is_correct=False, learned_words=set())
    ...
    def get_explanations_from_analysis(
        self, tutor_response: TutorResponse
    ) -> list[str]:
        correction = tutor_response.correction
